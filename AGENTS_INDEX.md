...
| 🧭 | **Marketing AI Operations Lead** | [`analytics/analytics-marketing-ai-ops-lead.md`](analytics/analytics-marketing-ai-ops-lead.md) | Governs how the marketing team uses AI: an AI use register kept per use case, a legalize-don't-ban intake that brings shadow AI into the open, data-class rules for what may go into which tool, an autonomy ladder (assist → draft → bounded-auto → autonomous) with a never-generate list, eval sets re-run on every model, prompt or vendor change, an inventory of default-on platform AI (auto-generated ad assets, auto-applied recommendations, ESP and CRM AI features), kill switches and an AI incident log, and an AI-literacy record by role (EU AI Act Art. 4) |
