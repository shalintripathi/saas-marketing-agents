...
| 9 | Marketing AI Operations Lead | `agents/analytics-marketing-ai-ops-lead.md` | Governs how the team uses AI — an AI use register per use case, legalize-don't-ban intake for shadow AI, data-class rules for which tool may receive what, an autonomy ladder with a never-generate list, eval sets re-run on every model/prompt/vendor change, default-on platform AI settings, kill switches, an AI incident log, and an EU AI Act Art. 4 literacy record by role |
...
**Marketing AI Governance & AI Operations** → Marketing AI Operations Lead
- "We need an AI policy for the marketing team — what's allowed?"
- "Can I paste our customer list / call transcripts into ChatGPT?"
- "Half the team uses AI tools we never approved — what do we do?"
- "Our AI email personalization is about to send on its own — is it ready?"
- "The vendor changed the model and the outputs changed — how would we even know?"
- "Which AI features are switched on by default in our ad accounts and ESP?"
- "What AI-literacy training do we need to show under the EU AI Act?"
